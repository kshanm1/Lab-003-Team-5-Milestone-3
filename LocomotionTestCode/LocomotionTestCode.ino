...
void forwardDistance(long distanceCm){
  //Use encoder positions to move a specific distance
    long targetCounts = distanceCm / distancePerCount;
  ...
  while (true) {
        ...
        Serial.printf("\nTarget|Left|Right:     %l    |    %l    |    %l", targetCounts, traveledLeftCounts, traveledRightCounts);
        // Check if the robot has traveled the desired distance
        if (traveledLeftCounts >= targetCounts && traveledRightCounts >= targetCounts) {
...
void ninetyLeft(){
  ...
  while (true) {
        ...
        long traveledRightCounts = abs(RightEncoder.lRawEncoderCount - initialRightCount);
        // Check if the robot has traveled the desired distance
        if (traveledRightCounts >= targetCounts) {
            break; // Exit loop if target distance is reached
        }
  }
